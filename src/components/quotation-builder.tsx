...
export function QuotationBuilder({ organizationId, canManage }: { organizationId: string | null; canManage: boolean }) {
  ...
  const openCostModel = (product: CatalogItem) => {
    ...
    setCostModelDraft(costComponents.filter((entry) => entry.product_catalog_item_id === product.id).map((entry) => {
      const resource = catalog.find((item) => item.id === entry.cost_catalog_item_id);
      return { rowId: entry.id, catalogItemId: entry.cost_catalog_item_id, quantity: String(entry.quantity), unitCostOverride: String(entry.unit_cost_override ?? resource?.default_unit_cost ?? 0) };
    }));
    setMessage("");
  };
  ...
  const addCostModelRow = () => {
    ...
    setCostModelDraft((current) => [...current, { rowId: crypto.randomUUID(), catalogItemId: resource.id, quantity: "1", unitCostOverride: String(resource.default_unit_cost) }]);
  };
  ...
  const saveCostModel = async () => {
    ...
    if (costModelDraft.some((entry) => entry.unitCostOverride.trim() === "" || !Number.isFinite(Number(entry.unitCostOverride)) || Number(entry.unitCostOverride) < 0)) return setMessage("Indica el costo aplicado a cada recurso, aunque sea cero.");
    setSaving(true); setMessage("");
    const response = await fetch("/api/quotations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "save_cost_model", organizationId, parentCatalogItemId: costModelProductId, components: costModelDraft.map((entry) => ({ catalogItemId: entry.catalogItemId, quantity: Number(entry.quantity), unitCostOverride: Number(entry.unitCostOverride) })) }) });
    setSaving(false);
  ...
  return <main className="dashboard quotation-workspace">
    ...
    {view === "builder" && <>
      <section className="panel quote-meta-panel">
        ...
        <div className="quote-meta-grid">
          <label>N.º de cotización<input value={draft.quoteNumber ?? "Se asigna al guardar"} readOnly aria-label="Número correlativo de cotización" /><small>correlativo automático</small></label>
    ...
    {costModelProduct && <div className="modal-backdrop"><section className="entry-modal quote-cost-model-modal" role="dialog" aria-modal="true" aria-labelledby="cost-model-title"><div className="modal-header"><div><span className="eyebrow">RECETA DE COSTOS</span><h2 id="cost-model-title">{costModelProduct.name}</h2><p>Selecciona lo que paga GEIMSER para operar una unidad de este producto.</p></div><button type="button" className="close-button" onClick={() => { setCostModelProductId(null); setCostModelDraft([]); }} aria-label="Cerrar">×</button></div>
      ...
      <div className="quote-cost-model-heading"><div><h3>Recursos asociados</h3><p>Prorratea aquí la factura global: por ejemplo, de USD 80 en Vercel puedes aplicar USD 25 sólo a Atlas Aprende.</p></div><button type="button" className="secondary-button" onClick={addCostModelRow}>Agregar recurso</button></div>
      {!costModelDraft.length ? <div className="quote-empty"><strong>Este producto todavía no tiene recursos asociados</strong><span>Agrega hosting, IA, perfiles u otro costo {costModelProduct.billing_period === "monthly" ? "recurrente" : "único"}.</span></div> : <div className="quote-cost-model-list">{costModelDraft.map((entry) => { const resource = catalog.find((item) => item.id === entry.catalogItemId); const effectiveCost = asNumber(entry.unitCostOverride); return <article key={entry.rowId}><label>Recurso<select value={entry.catalogItemId} onChange={(event) => { const nextResource = catalog.find((item) => item.id === event.target.value); updateCostModelRow(entry.rowId, { catalogItemId: event.target.value, unitCostOverride: String(nextResource?.default_unit_cost ?? 0) }); }}>{availableCostCatalog.filter((item) => item.id !== costModelProduct.id && item.billing_period === costModelProduct.billing_period).map((item) => <option key={item.id} value={item.id}>{item.name} · {item.billing_period === "monthly" ? "mensual" : "una vez"}</option>)}</select></label><label>Cantidad<input type="number" min="0.0001" step="0.0001" value={entry.quantity} onChange={(event) => updateCostModelRow(entry.rowId, { quantity: event.target.value })} /></label><label>Costo aplicado<input type="number" min="0" step="0.01" value={entry.unitCostOverride} onChange={(event) => updateCostModelRow(entry.rowId, { unitCostOverride: event.target.value })} /><small>Base catálogo: {new Intl.NumberFormat("es-CL", { maximumFractionDigits: 2 }).format(asNumber(resource?.default_unit_cost ?? 0))}</small></label><div className="quote-cost-model-subtotal"><span>Subtotal</span><strong>{new Intl.NumberFormat("es-CL", { maximumFractionDigits: 2 }).format(asNumber(entry.quantity) * effectiveCost)}</strong></div><button type="button" className="quote-remove" onClick={() => setCostModelDraft((current) => current.filter((item) => item.rowId !== entry.rowId))} aria-label={`Quitar ${resource?.name ?? "recurso"}`}>×</button></article>; })}</div>}

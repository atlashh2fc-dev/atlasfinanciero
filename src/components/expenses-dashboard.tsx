...
export function ExpensesDashboard({ organizationId, canManage, canConfigureSii }: { organizationId: string | null; canManage: boolean; canConfigureSii: boolean }) {
  ...
  return <main className="dashboard">
    ...
    <SiiDteIntegration organizationId={organizationId} canConfigure={canConfigureSii} documents={documents} onRefreshDocuments={() => { void refreshDocuments(); }} onOpenDocument={(documentId) => { const document = documents.find((item) => item.id === documentId); if (document) void openDocumentDetail(document); }} />

...
export function ExpensesDashboard({ organizationId }: { organizationId: string | null }) {
  ...
  const summary = useMemo(() => {
    const invoices = visible.filter(isInvoice);
    const paid = invoices.filter((document) => normal(document.payment_status).includes("pagada")).reduce((total, document) => total + amount(document.total_amount), 0);
    const expense = visible.reduce((total, document) => total + signedTotal(document), 0);
  ...
  const supplierSummary = useMemo(() => {
    ...
    for (const document of visible.filter((item) => !isGuide(item))) {
      const key = document.supplier_tax_id || document.supplier_name;
      const current = bySupplier.get(key) ?? { name: document.supplier_name, taxId: document.supplier_tax_id, documents: 0, total: 0 };
      current.documents += 1;
      current.total += signedTotal(document);
  ...
  return <main className="dashboard">
    ...
    <section className="table-section"><div className="table-heading"><div><span className="panel-label">FILTROS</span><h2>Detalle de facturas recibidas</h2><p>El filtro anual controla los KPI, proveedores y documentos del detalle.</p></div><button type="button" className="secondary-button" onClick={() => { setSupplier("all"); setStatus("all"); setSearch(""); }}>Limpiar filtros</button></div><div className="filter-row"><label>Proveedor<select value={supplier} onChange={(event) => setSupplier(event.target.value)}><option value="all">Todos los proveedores</option>{suppliers.map((item) => <option key={item.supplier_tax_id || item.supplier_name} value={item.supplier_tax_id || item.supplier_name}>{item.supplier_name}{item.supplier_tax_id ? ` · ${item.supplier_tax_id}` : ""}</option>)}</select></label><label>Estado<select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">Todos los estados</option>{statuses.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>Buscar<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Proveedor, RUT, folio o detalle" /></label></div>{loading ? <p className="billing-empty">Cargando documentos recibidos…</p> : <div className="table-scroll"><table><thead><tr><th>Documento</th><th>Proveedor</th><th>Emisión / vencimiento</th><th>Detalle</th><th className="money-col">Neto</th><th className="money-col">IVA</th><th className="money-col">Total</th><th>Pago</th></tr></thead><tbody>{visible.length ? visible.map((document) => <tr key={document.id}><td><strong>{document.document_type}</strong><small>Folio: {document.document_number || "—"}</small></td><td><strong>{document.supplier_name}</strong><small>{document.supplier_tax_id || "Sin RUT"}</small></td><td>{displayDate(document.issue_date)}<small>Vence: {displayDate(document.due_date)}</small></td><td><small>{document.notes || "Sin observación"}</small></td><td className="money-col">{money.format(amount(document.net_amount))}</td><td className="money-col">{money.format(amount(document.vat_amount) + amount(document.additional_tax_amount))}</td><td className={`money-col ${isCredit(document) ? "is-negative" : ""}`}>{isCredit(document) ? "−" : ""}{money.format(amount(document.total_amount))}</td><td><span className={statusClass(document.payment_status)}>{document.payment_status || "Sin estado"}</span><small>{[document.payment_method, displayDate(document.payment_date)].filter((item) => item && item !== "—").join(" · ") || "Sin pago registrado"}</small></td></tr>) : <tr><td colSpan={8}>No hay documentos para los filtros seleccionados.</td></tr>}</tbody></table></div>}</section>

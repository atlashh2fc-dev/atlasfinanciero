...
type ReceivedDocument = { id: string; supplier_counterparty_id: string | null; supplier_name: string; supplier_tax_id: string | null; document_number: string | null; issue_date: string; document_type: string; net_amount: number | string; vat_amount: number | string; additional_tax_amount: number | string; total_amount: number | string; notes: string | null; due_date: string | null; payment_status: string | null; payment_method: string | null; payment_bank: string | null; payment_reference: string | null; payment_date: string | null };
type DirectPayable = { id: string; payable_number: string; supplier_counterparty_id: string | null; supplier_name: string; invoice_number: string | null; category: string; category_detail: string | null; description: string; issue_date: string; due_date: string | null; total_amount: number | string; currency_code: string; status: "draft" | "review" | "approved" | "rejected" | "paid"; notes: string | null; payment_reference: string | null; paid_at: string | null };
type Payable = { id: string; source: "received" | "direct"; supplier_name: string; supplier_tax_id: string | null; document_number: string | null; issue_date: string; document_type: string; net_amount: number | string; vat_amount: number | string; additional_tax_amount: number | string; total_amount: number | string; currency_code: string; notes: string | null; due_date: string | null; payment_status: string | null; payment_method: string | null; payment_bank: string | null; payment_reference: string | null; payment_date: string | null; workflow_status: DirectPayable["status"] | null };
...
const statusClass = (status: string | null) => normal(status).includes("pagada") ? "status paid" : normal(status).includes("pendiente") ? "status pending" : normal(status).includes("rechazada") || normal(status).includes("nota") ? "status cancelled" : "status neutral";
const countsAsExpense = (item: Payable) => item.source === "received" || item.workflow_status === "approved" || item.workflow_status === "paid";
...
export function ExpensesDashboard({ organizationId, canManage }: { organizationId: string | null; canManage: boolean }) {
  ...
  const payables = useMemo<Payable[]>(() => [
    ...documents.map((document) => ({ ...document, source: "received" as const, currency_code: "CLP", workflow_status: null })),
    ...directPayables.map((payable) => ({ id: payable.id, source: "direct" as const, supplier_name: payable.supplier_name, supplier_tax_id: null, document_number: payable.invoice_number || payable.payable_number, issue_date: payable.issue_date, document_type: "Cuenta por pagar directa", net_amount: payable.total_amount, vat_amount: 0, additional_tax_amount: 0, total_amount: payable.total_amount, currency_code: payable.currency_code, notes: payable.notes || payable.description, due_date: payable.due_date, payment_status: directStatusLabel[payable.status], payment_method: payable.status === "paid" ? "Lote de pago" : null, payment_bank: null, payment_reference: payable.payment_reference, payment_date: payable.paid_at?.slice(0, 10) ?? null, workflow_status: payable.status })),
  ...
  const suppliers = useMemo(() => [...new Map(payables.map((item) => [item.supplier_tax_id || item.supplier_name, item])).values()].sort((a, b) => a.supplier_name.localeCompare(b.supplier_name, "es")), [payables]);
  ...
  const visible = useMemo(() => payables.filter((item) => {
    ...
    const matchesSupplier = supplier === "all" || (item.supplier_tax_id || item.supplier_name) === supplier;

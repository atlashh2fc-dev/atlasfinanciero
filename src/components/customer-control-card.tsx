...
type Customer = { id?: string; legal_name: string; trade_name: string | null; tax_id: string | null; related_ids?: string[]; aliases?: string[] };
...
type CustomerService = { id: string; counterparty_id: string; service_catalog_id: string | null; custom_service_name: string | null; custom_service_category: string | null; quantity: number; unit_price: number; currency: "CLP" | "UF" | "USD"; starts_on: string | null; ends_on: string | null; billing_frequency: "monthly" | "one_time" | "annual" | "quarterly"; notes: string | null; is_active: boolean };
...
export function CustomerControlCard({ organizationId, customer, canManage }: { organizationId: string | null; customer: Customer; canManage: boolean }) {
  ...
  useEffect(() => { setSelectedCategory(""); setSelectedCatalogId(""); setCustomServiceName(""); setServiceDescription(""); setQuantity("1"); setUnitPrice("0"); setCurrency("CLP"); setUfPricingDate(today()); setUfQuote(null); }, [customer.id]);
  ...
  const relatedCustomerIds = useMemo(() => new Set([customer.id, ...(customer.related_ids ?? [])].filter((item): item is string => Boolean(item))), [customer.id, customer.related_ids]);
  const clientNames = useMemo(() => new Set([customer.legal_name, customer.trade_name, ...(customer.aliases ?? [])].map(normalize).filter(Boolean)), [customer]);
  const issued = useMemo(() => (data?.issuedDocuments ?? []).filter((item) => Boolean(item.counterparty_id && relatedCustomerIds.has(item.counterparty_id)) || clientNames.has(normalize(item.client_name)) || clientNames.has(normalize(item.recipient_name))), [data, relatedCustomerIds, clientNames]);
  const received = useMemo(() => (data?.receivedDocuments ?? []).filter((item) => Boolean(item.supplier_counterparty_id && relatedCustomerIds.has(item.supplier_counterparty_id)) || clientNames.has(normalize(item.supplier_name))), [data, relatedCustomerIds, clientNames]);
  const services = useMemo(() => (data?.services ?? []).filter((item) => relatedCustomerIds.has(item.counterparty_id)), [data, relatedCustomerIds]);
  const files = useMemo(() => (data?.files ?? []).filter((item) => relatedCustomerIds.has(item.counterparty_id)), [data, relatedCustomerIds]);
  ...
  const catalogCategories = useMemo(() => [...new Set(availableCatalog.map((item) => item.category?.trim() || "Sin categoría"))].sort((left, right) => left.localeCompare(right, "es")), [availableCatalog]);
  const servicesInCategory = useMemo(() => availableCatalog.filter((item) => (item.category?.trim() || "Sin categoría") === selectedCategory), [availableCatalog, selectedCategory]);
  ...
  const saveService = async (event: FormEvent) => {
    event.preventDefault(); if (!organizationId || !customer.id || (!selectedCatalogId && !customServiceName.trim())) return;
    const response = await fetch("/api/customer-control", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "save_service", organizationId, item: { counterpartyId: customer.id, catalogId: selectedCatalogId === "__other__" ? null : selectedCatalogId, customServiceName: selectedCatalogId === "__other__" ? customServiceName : null, category: selectedCategory, quantity, unitPrice, currency, billingFrequency: frequency, notes: serviceDescription, isActive: true } }) });
    if (!response.ok) return setMessage("No fue posible guardar el servicio. Revisa cantidad y precio.");
    setMessage("Servicio contratado guardado."); setSelectedCategory(""); setSelectedCatalogId(""); setCustomServiceName(""); setServiceDescription(""); setQuantity("1"); await load();
  };
  ...
  return <section className="customer-control">
    ...
    {loading ? <p className="billing-empty">Cargando control del cliente…</p> : <div className="customer-control-grid">
      ...
      <article className="customer-control-section"><div className="section-title"><h4>Servicios contratados</h4><span>{services.length}</span></div>{services.length ? <div className="control-list">{services.map((service) => { const item = service.service_catalog_id ? catalogById.get(service.service_catalog_id) : null; const category = service.custom_service_category ?? item?.category; return <div key={service.id}><div><strong>{category ? `${category} · ` : ""}{service.custom_service_name ?? item?.name ?? "Servicio no disponible"}</strong><small>{service.quantity} × {service.currency} {money.format(Number(service.unit_price))}{service.currency === "UF" ? " · valorización SII en cada prefactura" : ""} · {service.billing_frequency === "monthly" ? "Mensual" : service.billing_frequency === "annual" ? "Anual" : service.billing_frequency === "quarterly" ? "Trimestral" : "Única vez"}</small>{service.notes && <small className="service-description">{service.notes}</small>}</div>{canManage && <button type="button" className="text-button" onClick={() => void removeService(service.id)}>Quitar</button>}</div>; })}</div> : <p className="control-empty">Sin servicios contratados.</p>}{canManage && <form className="service-assignment" onSubmit={saveService}><label>Categoría<select value={selectedCategory} onChange={(event) => chooseCategory(event.target.value)} required><option value="">Seleccionar categoría…</option>{catalogCategories.map((category) => <option key={category} value={category}>{category}</option>)}</select></label><label>Servicio<select value={selectedCatalogId} onChange={(event) => chooseCatalog(event.target.value)} disabled={!selectedCategory} required><option value="">{selectedCategory ? "Seleccionar servicio…" : "Elige primero categoría"}</option>{servicesInCategory.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}<option value="__other__">Otro…</option></select></label>{selectedCatalogId === "__other__" && <label className="service-custom-name">Nombre del servicio<input value={customServiceName} maxLength={180} onChange={(event) => setCustomServiceName(event.target.value)} placeholder="Ej. Soporte especial" required /></label>}<label>Cantidad<input type="number" min="0.0001" step="any" value={quantity} onChange={(event) => setQuantity(event.target.value)} required /></label><label>Precio pactado<input type="number" min="0" step={currency === "UF" ? "0.0001" : "1"} value={unitPrice} onChange={(event) => setUnitPrice(event.target.value)} required /></label><label>Moneda<select value={currency} onChange={(event) => setCurrency(event.target.value as "CLP" | "UF")}><option value="CLP">CLP</option><option value="UF">UF</option></select></label>{currency === "UF" && <label>Fecha UF SII<input type="date" value={ufPricingDate} onChange={(event) => setUfPricingDate(event.target.value)} required /></label>}<label>Frecuencia<select value={frequency} onChange={(event) => setFrequency(event.target.value as CustomerService["billing_frequency"])}><option value="monthly">Mensual</option><option value="quarterly">Trimestral</option><option value="annual">Anual</option><option value="one_time">Única vez</option></select></label><label className="service-description-field">Descripción breve<textarea value={serviceDescription} maxLength={500} onChange={(event) => setServiceDescription(event.target.value)} placeholder="Alcance, condiciones o detalle corto del servicio" /></label>{currency === "UF" && <p className="uf-reference-note">{loadingUf ? "Consultando UF oficial…" : ufQuote ? `SII · ${ufQuote.date} · 1 UF = CLP ${money.format(ufQuote.value)} · Referencia: CLP ${money.format(Number(unitPrice || 0) * ufQuote.value)}.` : "No hay valor UF del SII disponible para esta fecha."}</p>}<button className="primary-button" type="submit" disabled={(selectedCatalogId === "__other__" && !customServiceName.trim()) || (currency === "UF" && (!ufQuote || loadingUf))}>Agregar servicio</button></form>}</article>

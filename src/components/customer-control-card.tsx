...
export function CustomerControlCard({ organizationId, customer, canManage }: { organizationId: string | null; customer: Customer; canManage: boolean }) {
  ...
  useEffect(() => { setSelectedCategory(""); setSelectedCatalogId(""); setQuantity("1"); setUnitPrice("0"); setCurrency("CLP"); setUfPricingDate(today()); setUfQuote(null); }, [customer.id]);
  ...
  const saveService = async (event: FormEvent) => {
    ...
    setMessage("Servicio contratado guardado."); setSelectedCategory(""); setSelectedCatalogId(""); setQuantity("1"); await load();
  };

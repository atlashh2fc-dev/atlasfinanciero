...
type CatalogItem = { id: string; name: string; category: string | null; unit_name: string; unit_price: number; currency: "CLP" | "USD" | "UF"; is_active: boolean };
type CustomerService = { id: string; counterparty_id: string; service_catalog_id: string; quantity: number; unit_price: number; currency: "CLP" | "USD" | "UF"; starts_on: string | null; ends_on: string | null; billing_frequency: "monthly" | "one_time" | "annual" | "quarterly"; notes: string | null; is_active: boolean };
...
export function CustomerControlCard({ organizationId, customer, canManage }: { organizationId: string | null; customer: Customer; canManage: boolean }) {
  ...
  useEffect(() => { setSelectedCatalogId(""); setQuantity("1"); setUnitPrice("0"); }, [customer.id]);
  ...
  return <section className="customer-control">
    ...
    {loading ? <p className="billing-empty">Cargando control del cliente…</p> : <div className="customer-control-grid">
      ...
      <article className="customer-control-section"><div className="section-title"><h4>Servicios contratados</h4><span>{services.length}</span></div>{services.length ? <div className="control-list">{services.map((service) => { const item = catalogById.get(service.service_catalog_id); return <div key={service.id}><div><strong>{item?.name ?? "Servicio no disponible"}</strong><small>{service.quantity} × {service.currency} {money.format(Number(service.unit_price))} · {service.billing_frequency === "monthly" ? "Mensual" : service.billing_frequency === "annual" ? "Anual" : service.billing_frequency === "quarterly" ? "Trimestral" : "Única vez"}</small></div>{canManage && <button type="button" className="text-button" onClick={() => void removeService(service.id)}>Quitar</button>}</div>; })}</div> : <p className="control-empty">Sin servicios contratados.</p>}{canManage && <form className="service-assignment" onSubmit={saveService}><label>Servicio<select value={selectedCatalogId} onChange={(event) => chooseCatalog(event.target.value)} required><option value="">Seleccionar…</option>{availableCatalog.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Cantidad<input type="number" min="0.0001" step="any" value={quantity} onChange={(event) => setQuantity(event.target.value)} required /></label><label>Precio pactado<input type="number" min="0" step="1" value={unitPrice} onChange={(event) => setUnitPrice(event.target.value)} required /></label><label>Frecuencia<select value={frequency} onChange={(event) => setFrequency(event.target.value as CustomerService["billing_frequency"])}><option value="monthly">Mensual</option><option value="quarterly">Trimestral</option><option value="annual">Anual</option><option value="one_time">Única vez</option></select></label><button className="primary-button" type="submit">Agregar servicio</button></form>}</article>

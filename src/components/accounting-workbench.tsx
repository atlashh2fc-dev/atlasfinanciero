...
export function AccountingWorkbench({ organizationId }: { organizationId: string | null }) {
  ...
  return <main className="dashboard billing-dashboard">
    ...
    {loading ? <section className="panel billing-empty"><p>Cargando libros contables…</p></section> : !data ? <section className="panel billing-empty"><p>Selecciona una empresa para consultar su contabilidad.</p></section> : <>
      ...
      <section className="table-section"><div className="table-heading"><div><span className="panel-label">PLAN DE CUENTAS</span><h2>Catálogo contable</h2><p>Base IFRS / Chile editable. El saldo normal orienta la presentación, pero cada línea de un asiento puede ir al Debe o al Haber según la operación.</p></div></div><div className="table-scroll"><table className="billing-cycles-table"><thead><tr><th>Código</th><th>Cuenta</th><th>Naturaleza</th><th>Saldo normal</th><th>Presentación</th></tr></thead><tbody>{data.accounts.map((account) => <tr key={account.id}><td><strong>{account.account_code}</strong></td><td>{account.account_name}</td><td>{natureLabels[account.nature]}</td><td>{account.normal_balance === "debit" ? "Deudor" : "Acreedor"}</td><td>{account.presentation_group || areaLabels[account.statement_area]}</td></tr>)}</tbody></table></div></section>

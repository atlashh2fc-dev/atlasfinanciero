...
function sumRecognizedNet(records: InvoiceRecord[]) {
  return records.reduce((total, record) => total + recognizedNetAmount(record), 0);
}
...
function ExecutiveDashboard({ records }: { records: InvoiceRecord[] }) {
  ...
  const pending = records.filter((record) => !isPurchaseOrderDocument(record) && !isCreditNoteDocument(record) && record.status?.toLowerCase().includes("pendiente"));
  ...
  const monthlyExecutive = calendarMonths.map((month, index) => ({
    ...
    documented: index < currentMonthIndex ? sumRecognizedNet(closedMonthRecords.filter((record) => record.month === month)) : null,
...
function CustomerModule({ records, organizationId, canManage }: { records: InvoiceRecord[]; organizationId: string | null; canManage: boolean }) {
  const [isEvolutionExpanded, setIsEvolutionExpanded] = useState(false);
  const customers = buildCustomerEvolution(records);
  const totalNet = sumRecognizedNet(records);
  ...
  return (
    <main className="dashboard customer-dashboard">
      <section className="headline">
        <div><span className="eyebrow">ANÁLISIS COMERCIAL · 2026</span><h1>Clientes y evolución</h1><p>Facturas y exentos, menos notas de crédito. Las órdenes de compra no se contabilizan como ingreso facturado.</p></div>
...
type StoredDocument = {
  id: string; document_number: string | null; issue_date: string | null; document_type: string | null; issuer_name: string | null; issuer_tax_id: string | null; client_name: string | null; recipient_name: string | null; recipient_tax_id: string | null; net_amount: number | null; vat_amount: number | null; total_amount: number | null; notes: string | null; payment_term_days: number | null; due_date: string | null; due_month: string | null; payment_status: string | null; payment_date: string | null; payment_method: string | null; factoring_entity: string | null; factored_at: string | null; factoring_settled_at: string | null; factoring_recourse_at: string | null; origin_account_or_tax_id: string | null; destination_bank: string | null; destination_account: string | null; source_file_name: string | null; source_sheet_name: string | null; source_row: number | null;
};
...
export function FinanceDashboard() {
  ...
  const [editDraft, setEditDraft] = useState<DocumentUpdateDraft>({ status: "", paymentDate: "", paymentMethod: "", notes: "", factoringEntity: "", factoredAt: "", factoringSettledAt: "", factoringRecourseAt: "" });
  ...
  const availableYears = useMemo(
    () => Array.from(new Set(records.map((record) => record.year).filter((value): value is number => typeof value === "number"))).sort((first, second) => second - first),
    [records],
  );
  const yearFilteredRecords = useMemo(
    () => year === "Todos" ? records : records.filter((record) => record.year === Number(year)),
    [records, year],
  );
  const months = useMemo(
    () => calendarMonths.filter((item) => yearFilteredRecords.some((record) => record.month === item)),
    [yearFilteredRecords],
  );
  const statuses = useMemo(() => {
    const values = yearFilteredRecords.map((record) => record.status).filter((item): item is string => Boolean(item));
    return Array.from(new Set(values)).sort();
  }, [yearFilteredRecords]);
  const filtered = useMemo(
    () => yearFilteredRecords.filter((record) => (month === "Todos" || record.month === month) && (status === "Todos" || record.status === status)),
    [yearFilteredRecords, month, status],
  );

  const monthly = useMemo(
    () => months.map((item) => {
      const matching = yearFilteredRecords.filter((record) => record.month === item);
      return { month: item.slice(0, 3), montoNeto: sumRecognizedNet(matching), documentos: matching.filter((record) => !isPurchaseOrderDocument(record)).length };
    }),
    [months, yearFilteredRecords],
  );
  const statusesChart = useMemo(() => statuses.map((item) => ({
    name: item,
    value: yearFilteredRecords.filter((record) => record.status === item).length,
  })), [yearFilteredRecords, statuses]);
  const clientRanking = useMemo(() => Object.values(yearFilteredRecords.reduce<Record<string, { client: string; montoNeto: number }>>((accumulator, record) => {
    const client = record.client || "No informado";
    accumulator[client] ??= { client, montoNeto: 0 };
    if (!isPurchaseOrderDocument(record)) accumulator[client].montoNeto += recognizedNetAmount(record);
    return accumulator;
  }, {})).sort((a, b) => b.montoNeto - a.montoNeto).slice(0, 6), [yearFilteredRecords]);

  const pendingCount = yearFilteredRecords.filter((record) => !isPurchaseOrderDocument(record) && !isCreditNoteDocument(record) && record.status === "Pendiente").length;
  ...
  function startDocumentEdit(record: InvoiceRecord) {
    ...
    setEditDraft({ status: record.status ?? "", paymentDate: record.paymentDate ?? "", paymentMethod: record.paymentMethod ?? "", notes: record.notes ?? "", factoringEntity: record.factoringEntity ?? "", factoredAt: record.factoredAt ?? "", factoringSettledAt: record.factoringSettledAt ?? "", factoringRecourseAt: record.factoringRecourseAt ?? "" });
    setFormError("");
  }
  ...
  async function submitDocumentEdit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setFormError("No fue posible actualizar el documento. Revisa tu sesión y permisos.");
      return;
    }
    const payload = await response.json() as { document: StoredDocument };
    const updated = mapStoredDocument(payload.document);
    setDatabaseRecords((current) => current ? current.map((record) => record.id === updated.id ? updated : record) : current);
  ...
  async function submitEntry(event: FormEvent<HTMLFormElement>) {
    ...
    const created = mapStoredDocument({ ...payload.document, notes: null, payment_term_days: null, due_date: null, due_month: null, payment_date: null, payment_method: null, factoring_entity: null, factored_at: null, factoring_settled_at: null, factoring_recourse_at: null, origin_account_or_tax_id: null, destination_bank: null, destination_account: null, source_file_name: payload.document.source_file_name, source_sheet_name: payload.document.source_sheet_name, source_row: payload.document.source_row });
  ...
  return (
    <div className="app-shell">
      ...
      <section className="content-area">
        ...
        {activeModule === "Inicio" ? <ExecutiveDashboard records={records} /> : activeModule === "Órdenes de compra" ? <CustomerPurchaseOrders organizationId={access?.membership.organizationId ?? null} records={records} canManage={hasEditPermission} /> : activeModule === "Proyecciones" ? <ForecastModule /> : activeModule === "Clientes" ? <CustomerModule records={records} organizationId={access?.membership.organizationId ?? null} canManage={hasEditPermission} /> : activeModule === "Cuentas por cobrar" ? <AccountsReceivable records={records} organizationId={access?.membership.organizationId ?? null} canManage={hasEditPermission} isPersisted={Boolean(databaseRecords)} onEditDocument={(record) => { setActiveModule("Facturas"); startDocumentEdit(record); }} /> : activeModule === "Recurrentes" ? <BillingOperations /> : activeModule === "Remuneraciones" ? <PayrollDashboard organizationId={access?.membership.organizationId ?? null} canSynchronize={access?.membership.role === "administrator"} /> : activeModule === "Centros de costo" ? (canManageCostCenters ? <CostCenterManagement organizationId={access?.membership.organizationId ?? null} /> : null) : activeModule === "Reportes" ? <ReportsDashboard organizationId={access?.membership.organizationId ?? null} /> : activeModule === "Administración" ? (access?.membership.role === "administrator" ? <AdministrationConsole activeOrganizationId={access.membership.organizationId} /> : null) : activeModule !== "Facturas" ? <EmptyModule module={activeModule} /> : (
          <main className="dashboard">
            <section className="headline">
              <div><span className="eyebrow">OPERACIÓN · {year === "Todos" ? "TODOS LOS AÑOS" : year}</span><h1>Facturas emitidas</h1><p>Gestión documental, estados, vencimientos y trazabilidad por documento.</p></div>
              <div className="headline-actions">
                <label className="period-picker">Año<select value={year} onChange={(event) => { setYear(event.target.value); setMonth("Todos"); setStatus("Todos"); }}><option value="Todos">Todos los años</option>{availableYears.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
            ...
            <section className="kpis kpis-five" aria-label="Indicadores principales">
              <article className="kpi-card" data-help="Cantidad de documentos emitidos para el año seleccionado. Incluye facturas, exentos, notas de crédito y órdenes de compra registradas."><span>Documentos emitidos</span><strong>{number.format(yearFilteredRecords.length)}</strong><small>{databaseRecords ? `Año ${year === "Todos" ? "seleccionado" : year}` : sessionRecords.length ? `${sessionRecords.length} registro(s) guardado(s) en Atlas` : "Año 2026"}</small></article>
            ...
            <section className="table-section" id="facturas">
              ...
              <div className="table-scroll"><table><thead><tr><th>Documento</th><th>Emisión</th><th>Cliente</th><th>Tipo</th><th className="money-col">Neto/exento</th><th>Estado</th><th>Origen</th><th>Acción</th></tr></thead><tbody>{filtered.map((record) => <tr key={record.id}><td><strong>N° {record.invoiceNumber ?? "—"}</strong><small>{record.issuer}</small></td><td>{formatDate(record.issueDate)}<small>{record.month ?? "—"}</small></td><td><strong>{record.client ?? "No informado"}</strong><small>{record.recipient ?? "—"}</small></td><td>{record.documentType ?? "—"}</td><td className={`money-col ${recognizedNetAmount(record) < 0 ? "is-negative" : ""}`}>{formatMoney(recognizedNetAmount(record))}</td><td><span className={statusClass(record.status)}>{record.status ?? "No informado"}</span></td><td><span className="origin">{record.source.sheet}<b>fila {record.source.row || "sesión"}</b></span></td><td>{hasEditPermission && databaseRecords ? <button type="button" className="secondary-button" onClick={() => startDocumentEdit(record)}>Editar</button> : "—"}</td></tr>)}</tbody></table></div>
            </section>
          </main>
        )}
      </section>
      ...
      {editingRecord && <div className="modal-backdrop" role="presentation"><section className="entry-modal" role="dialog" aria-modal="true" aria-labelledby="edit-document-title"><div className="modal-header"><div><span className="eyebrow">COBRANZA Y PAGO</span><h2 id="edit-document-title">Documento N° {editingRecord.invoiceNumber ?? "—"}</h2><p>Actualiza el estado real, pago directo o ciclo de factoring sin alterar el monto neto/exento original.</p></div><button type="button" className="close-button" onClick={() => { setEditingRecord(null); setFormError(""); }} aria-label="Cerrar">×</button></div><form onSubmit={submitDocumentEdit}><div className="form-grid"><label>Estado *<select value={editDraft.status} onChange={(event) => setEditDraft((current) => ({ ...current, status: event.target.value }))}><option value="Pendiente">Pendiente</option><option value="Pagada">Pagada</option><option value="Factorizada">Factorizada</option><option value="Pagada al factoring">Pagada al factoring</option><option value="Recomprada al factoring">Recomprada al factoring</option><option value="Anulada">Anulada</option><option value="Nota de credito">Nota de crédito</option></select></label><label>Fecha pago directo<input type="date" value={editDraft.paymentDate} onChange={(event) => setEditDraft((current) => ({ ...current, paymentDate: event.target.value }))} /></label><label>Medio de pago<input value={editDraft.paymentMethod} onChange={(event) => setEditDraft((current) => ({ ...current, paymentMethod: event.target.value }))} placeholder="Transferencia, depósito…" /></label>{["Factorizada", "Pagada al factoring", "Recomprada al factoring"].includes(editDraft.status) && <><label>Entidad de factoring *<input value={editDraft.factoringEntity} onChange={(event) => setEditDraft((current) => ({ ...current, factoringEntity: event.target.value }))} placeholder="Nombre de la entidad" /></label><label>Fecha cesión / factoraje<input type="date" value={editDraft.factoredAt} onChange={(event) => setEditDraft((current) => ({ ...current, factoredAt: event.target.value }))} /></label><label>Fecha pago al factoring<input type="date" value={editDraft.factoringSettledAt} onChange={(event) => setEditDraft((current) => ({ ...current, factoringSettledAt: event.target.value }))} /></label><label>Fecha recompra GEIMSER<input type="date" value={editDraft.factoringRecourseAt} onChange={(event) => setEditDraft((current) => ({ ...current, factoringRecourseAt: event.target.value }))} /></label></>}<label>Observación<input value={editDraft.notes} onChange={(event) => setEditDraft((current) => ({ ...current, notes: event.target.value }))} /></label></div>{formError && <p className="form-error">{formError}</p>}<div className="form-actions"><button type="button" className="secondary-button" onClick={() => { setEditingRecord(null); setFormError(""); }}>Cancelar</button><button type="submit" className="primary-button">Guardar actualización</button></div></form></section></div>}
    </div>
  );
}

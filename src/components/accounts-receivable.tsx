...
function daysOverdue(dueDate: string | null, today: Date) {
  ...
  return Math.floor((today.getTime() - new Date(`${dueDate}T00:00:00`).getTime()) / 86400000);
}
...
export function AccountsReceivable({ records, organizationId, canManage, isPersisted }: { records: InvoiceRecord[]; organizationId: string | null; canManage: boolean; isPersisted: boolean }) {
  ...
  async function loadFollowups() {
    ...
    setIsLoading(true);
    const response = await fetch(`/api/collections/followups?organizationId=${encodeURIComponent(organizationId)}`, { cache: "no-store" });
    const payload = response.ok ? await response.json() as { followups: CollectionFollowup[] } : null;
    setFollowups(payload?.followups ?? []);
    if (!response.ok) setMessage("No fue posible cargar las gestiones de cobranza.");
    setIsLoading(false);
  }
  ...
  async function saveFollowup(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/collections/followups", {
      ...
      body: JSON.stringify({ organizationId, issuedDocumentId: selected.id, status: followupStatus, responsibleName, nextActionOn, promisedPaymentDate, note }),
    });
    setIsSaving(false);
    if (!response.ok) return setMessage("No fue posible guardar la gestión. Revisa tus permisos y vuelve a intentarlo.");
  ...
  return (
    <main className="dashboard receivables-dashboard">
      ...
      {!isPersisted && <p className="operation-message">La vista está en modo de respaldo local: la gestión se habilita cuando los documentos estén disponibles desde Atlas.</p>}

      <section className="kpis kpis-five" aria-label="Indicadores de cobranza">
        <article className="kpi-card"><span>Cuentas por cobrar</span><strong>{money.format(totalPending)}</strong><small>{receivables.length} documento(s) con estado pendiente</small></article>
      ...
      <section className="table-section">
        <div className="table-heading"><div><span className="panel-label">CARTERA OPERATIVA</span><h2>Documentos pendientes</h2><p>{isLoading ? "Cargando gestión…" : `${receivables.length} documento(s) priorizados por vencimiento.`}</p></div></div>
      ...
      {selected && <div className="modal-backdrop" role="presentation"><section className="entry-modal collection-modal" role="dialog" aria-modal="true" aria-labelledby="collection-title"><div className="modal-header"><div><span className="eyebrow">GESTIÓN DE COBRANZA</span><h2 id="collection-title">Factura N° {selected.invoiceNumber ?? "—"}</h2><p>{selected.client ?? "Cliente no informado"} · {money.format(selected.netAmount ?? 0)}</p></div><button type="button" className="close-button" onClick={() => setSelected(null)} aria-label="Cerrar">×</button></div><form onSubmit={saveFollowup}><div className="form-grid"><label>Estado<select value={followupStatus} onChange={(event) => setFollowupStatus(event.target.value as FollowupStatus)}><option value="open">Abierta</option><option value="committed">Compromiso de pago</option><option value="resolved">Resuelta</option></select></label><label>Responsable<input value={responsibleName} maxLength={180} onChange={(event) => setResponsibleName(event.target.value)} placeholder="Nombre responsable" /></label><label>Próxima acción<input type="date" value={nextActionOn} onChange={(event) => setNextActionOn(event.target.value)} /></label><label>Fecha compromiso de pago<input type="date" value={promisedPaymentDate} onChange={(event) => setPromisedPaymentDate(event.target.value)} /></label><label className="collection-note">Nota de gestión<textarea value={note} maxLength={2000} onChange={(event) => setNote(event.target.value)} placeholder="Contacto, acuerdo o próximo paso" /></label></div><div className="form-actions"><button type="button" className="secondary-button" onClick={() => setSelected(null)}>Cancelar</button><button type="submit" className="primary-button" disabled={isSaving}>{isSaving ? "Guardando…" : "Guardar gestión"}</button></div></form></section></div>}
    </main>
  );
}

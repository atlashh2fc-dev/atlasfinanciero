...
type IssuedDocument = { id: string; counterparty_id: string | null; document_number: string | null; issue_date: string | null; due_date: string | null; payment_term_days: number | null; document_type: string | null; net_amount: number | string | null; total_amount: number | string | null; outstanding_amount: number | string; payment_status: string | null; is_collectible: boolean; client_name: string | null };
...
export function FinancialPlanningDashboard({ organizationId }: { organizationId: string | null }) {
  ...
  }), [activeBudgetLines, data?.amortizationSchedules, data?.directPayables, data?.issuedDocuments, data?.receivedDocuments, year]);
  const totals = useMemo(() => monthly.reduce((total, line) => ({ actualRevenue: total.actualRevenue + line.actualRevenue, actualExpense: total.actualExpense + line.actualExpense, budgetRevenue: total.budgetRevenue + line.budgetRevenue, budgetExpense: total.budgetExpense + line.budgetExpense, assetAmortization: total.assetAmortization + line.assetAmortization, financedCash: total.financedCash + line.financedCash, financedCashUf: total.financedCashUf + line.financedCashUf }), { actualRevenue: 0, actualExpense: 0, budgetRevenue: 0, budgetExpense: 0, assetAmortization: 0, financedCash: 0, financedCashUf: 0 }), [monthly]);
  ...
  const cash = useMemo(() => {
    ...
    const weeks = Array.from({ length: data.settings.horizon_weeks }, (_, index) => {
      ...
      const inflows = data.issuedDocuments.filter((document) => document.is_collectible && !isPaid(document.payment_status) && number(document.outstanding_amount) > 0 && belongs(dueDate(document))).reduce((sum, document) => sum + Math.max(0, number(document.outstanding_amount)), 0)
  ...
  return <main className="dashboard reports-dashboard">
    ...
    <section className="table-section"><div className="table-heading"><div><span className="panel-label">PRESUPUESTO Y REAL</span><h2>Versión presupuestaria</h2><p>Una sola versión puede estar activa por año; los borradores permiten preparar escenarios sin afectar la comparación oficial.</p></div></div>
      ...
      <div className="table-scroll"><table><thead><tr><th>Mes</th><th className="money-col">Ingresos reales</th><th className="money-col">Plan ingresos</th><th className="money-col">Gastos reales</th><th className="money-col">Plan gastos</th><th className="money-col">Amortización activo</th><th className="money-col">Cuotas CLP</th><th className="money-col">Cuotas UF</th><th className="money-col">Resultado plan</th></tr></thead><tbody>{monthly.map((line) => <tr key={line.period}><td>{new Intl.DateTimeFormat("es-CL", { month: "long" }).format(new Date(`${line.period}T00:00:00`))}</td><td className="money-col">{money.format(line.actualRevenue)}</td><td className="money-col">{money.format(line.budgetRevenue)}</td><td className="money-col">{money.format(line.actualExpense)}</td><td className="money-col">{money.format(line.budgetExpense)}</td><td className="money-col">{money.format(line.assetAmortization)}</td><td className="money-col">{money.format(line.financedCash)}</td><td className="money-col">{line.financedCashUf ? `${line.financedCashUf.toLocaleString("es-CL", { maximumFractionDigits: 4 })} UF` : "—"}</td><td className={`money-col ${line.budgetRevenue - line.budgetExpense - line.assetAmortization < 0 ? "is-negative" : ""}`}>{money.format(line.budgetRevenue - line.budgetExpense - line.assetAmortization)}</td></tr>)}</tbody></table></div>

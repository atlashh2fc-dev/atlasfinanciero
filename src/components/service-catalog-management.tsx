...
type CatalogDraft = Omit<CatalogItem, "unit_price"> & { unit_price: string };
const blank = (): CatalogDraft => ({ name: "", category: "", description: "", unit_name: "unidad", unit_price: "", currency: "CLP", is_active: true });
...
export function ServiceCatalogManagement({ organizationId }: { organizationId: string }) {
  ...
  const set = (field: keyof CatalogDraft, value: string | boolean) => setDraft((current) => current ? { ...current, [field]: value } as CatalogDraft : current);
  const save = async (event: FormEvent) => {
    event.preventDefault(); if (!draft) return;
    const unitPrice = Number(draft.unit_price);
    if (!draft.unit_price.trim() || !Number.isFinite(unitPrice) || unitPrice < 0) { setMessage("Ingresa un precio base válido."); return; }
    setSaving(true); setMessage("");
    const response = await fetch("/api/customer-control", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "save_catalog", organizationId, item: { id: draft.id, name: draft.name, category: draft.category, description: draft.description, unitName: draft.unit_name, unitPrice, currency: draft.currency, isActive: draft.is_active } }) });
    setSaving(false);

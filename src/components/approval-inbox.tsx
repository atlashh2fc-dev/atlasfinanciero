...
type DirectPayableDetails = { supplierName: string; beneficiaryName: string | null; invoiceNumber: string | null; category: string | null; categoryDetail: string | null; description: string; issueDate: string | null; dueDate: string | null; notes: string | null; costCenter: { code: string; name: string } | null; attachments: { id: string; fileName: string; signedUrl: string | null }[] };
...
function directPayableDetails(metadata: Record<string, unknown>): DirectPayableDetails | null {
  ...
  const attachments = Array.isArray(detail.attachments)
    ? detail.attachments.flatMap((attachment) => attachment && typeof attachment === "object" && !Array.isArray(attachment) && typeof (attachment as Record<string, unknown>).id === "string" && typeof (attachment as Record<string, unknown>).fileName === "string"
      ? [{ id: (attachment as Record<string, string>).id, fileName: (attachment as Record<string, string>).fileName, signedUrl: typeof (attachment as Record<string, unknown>).signedUrl === "string" ? (attachment as Record<string, string>).signedUrl : null }]
      : [])
    : [];
  return description ? { supplierName: text("supplier_name") ?? "Proveedor no informado", beneficiaryName: text("beneficiary_name"), invoiceNumber: text("invoice_number"), category: text("category"), categoryDetail: text("category_detail"), description, issueDate: text("issue_date"), dueDate: text("due_date"), notes: text("notes"), costCenter, attachments } : null;
}

function directPayableCategory(category: string | null, detail: string | null) {
  if (category === "other") return detail ? `Otro · ${detail}` : "Otro";
  return ({ utilities: "Servicios básicos", rent: "Arriendo", taxes: "Impuestos / contribuciones", insurance: "Seguros", subscriptions: "Suscripciones", termination: "Finiquito" } as Record<string, string>)[category ?? ""] ?? "No informado";
}

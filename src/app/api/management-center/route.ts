...
export async function GET(request: NextRequest) {
  ...
  const issuedRows = (issued.data ?? []).map((item) => ({
    ...item,
    id: item.issued_document_id,
    total_amount: Number(item.outstanding_amount ?? 0),
    payment_status: item.collection_status,
  })); const receivedRows = received.data ?? []; const directRows = (direct.data ?? []).map((item) => ({ ...item, total_amount: Math.max(0, Number(item.total_amount ?? 0) - (paidByDirectPayable.get(item.id) ?? 0)) }));

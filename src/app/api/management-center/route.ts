...
export async function GET(request: NextRequest) {
  ...
  const [issued, received, direct, approvals, executions, billingAlerts] = await Promise.all([
  ...
  if ([issued, received, direct, approvals, executions, billingAlerts].some((result) => result.error)) return NextResponse.json({ error: "unable_to_load_management_center" }, { status: 500 });
  const issuedRows = issued.data ?? []; const receivedRows = received.data ?? []; const directRows = direct.data ?? [];

...
export async function GET(request: NextRequest) {
  ...
  const [issued, issuedPayments, received, direct, directPayments, approvals, executions, billingAlerts] = await Promise.all([
  ...
  if ([issued, issuedPayments, received, direct, directPayments, approvals, executions, billingAlerts].some((result) => result.error)) return NextResponse.json({ error: "unable_to_load_management_center" }, { status: 500 });
  ...
  (directPayments.data ?? []).forEach((payment) => {
    ...
    paidByDirectPayable.set(payment.direct_payable_id, (paidByDirectPayable.get(payment.direct_payable_id) ?? 0) + Number(payment.amount ?? 0));
  });
  const issuedRows = (issued.data ?? []).map((item) => ({ ...item, total_amount: Math.max(0, Number(item.total_amount ?? 0) - (paidByIssuedDocument.get(item.id) ?? 0)) })); const receivedRows = received.data ?? []; const directRows = (direct.data ?? []).map((item) => ({ ...item, total_amount: Math.max(0, Number(item.total_amount ?? 0) - (paidByDirectPayable.get(item.id) ?? 0)) }));
  ...
  const payables = [
    ...
    ...directRows.filter((item) => item.status === "approved" && Number(item.total_amount) > 0.01 && item.due_date && item.due_date < today).map((item) => ({ ...item, source: "direct" })),
  ];

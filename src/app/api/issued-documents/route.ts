...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json().catch(() => null) as DocumentRequest | null;
  ...
  if (!issueDate || !documentType || !issuer || !client || netAmount === undefined || totalAmount === undefined || vatAmount === undefined || !status) {
  ...
  const { data, error } = await supabase
    ...
    .select("id, document_number, issue_date, document_type, issuer_name, issuer_tax_id, client_name, recipient_name, recipient_tax_id, net_amount, vat_amount, total_amount, payment_status, source_file_name, source_sheet_name, source_row")
    .single();

  if (error) return NextResponse.json({ error: "unable_to_create_document" }, { status: 403 });
...
export async function PATCH(request: NextRequest) {
  ...
  const paymentDate = paymentDateValue === "" || paymentDateValue === null || paymentDateValue === undefined ? null : readDate(paymentDateValue);
  const factoredAt = body?.factoredAt ? readDate(body.factoredAt) : null;
  const factoringSettledAt = body?.factoringSettledAt ? readDate(body.factoringSettledAt) : null;
  const factoringRecourseAt = body?.factoringRecourseAt ? readDate(body.factoringRecourseAt) : null;
  if (!isUuid(documentId) || !status || (paymentDateValue && !paymentDate) || (body?.factoredAt && !factoredAt) || (body?.factoringSettledAt && !factoringSettledAt) || (body?.factoringRecourseAt && !factoringRecourseAt)) return NextResponse.json({ error: "invalid_document_update" }, { status: 400 });
  if ((status === "Factorizada" || status === "Pagada al factoring" || status === "Recomprada al factoring") && !readText(body?.factoringEntity, 180, true)) return NextResponse.json({ error: "factoring_entity_required" }, { status: 400 });
  ...
  const eligibleOrganizationIds = (memberships ?? []).filter((membership) => writeRoles.has(membership.role)).map((membership) => membership.organization_id);
  if (!eligibleOrganizationIds.length) return NextResponse.json({ error: "document_write_not_authorized" }, { status: 403 });

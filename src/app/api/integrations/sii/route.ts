...
export async function GET(request: NextRequest) {
  ...
  const [integration, events, syncRuns, rcvRuns, rcvPending] = await Promise.all([
    ...
    context.supabase.from("sii_rcv_sync_runs").select("started_at, completed_at, run_status, trigger_source, periods, purchase_entries_fetched, sale_entries_fetched, purchases_created, purchases_linked, sales_created, sales_linked, discrepancies, error_detail").eq("organization_id", organizationId).order("started_at", { ascending: false }).limit(30),
    context.supabase.from("sii_rcv_entries").select("id, operation, period, document_type, folio, counterpart_tax_id, counterpart_name, total_amount, match_status, match_detail").eq("organization_id", organizationId).in("match_status", ["unmatched", "amount_mismatch"]).order("updated_at", { ascending: false }).limit(200),
  ]);
  ...
  return NextResponse.json({
    ...
    events: events.data ?? [],
    mailSyncRuns: syncRuns.data ?? [],
    rcvSyncRuns: rcvRuns.error ? [] : rcvRuns.data ?? [],
    rcvPendingEntries: rcvPending.error ? [] : rcvPending.data ?? [],
    certificateConfigured: Boolean(process.env.SII_PRIVATE_KEY_PEM && process.env.SII_CERTIFICATE_PEM),
  });
}
